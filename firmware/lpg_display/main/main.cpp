...
static void showDisplaySelfTest() {
    ...
    lv_label_set_text(sub, "LCD · RGB · Backlight · LVGL");
    ...
    lv_label_set_text(foot, "LPG Filling Station v1.0");
...
// ── UI task (core 1, 32 KB stack) ─────────────────────────────────────────────
static void uiTask(void*) {
    showDisplaySelfTest();
    vTaskDelay(pdMS_TO_TICKS(2500));

    screenManager.begin(modbusClient, wifiManager);

    for (;;) {
        const ControllerSnapshot& snap = modbusClient.snapshot();
        screenManager.update(snap, modbusClient, wifiManager);
