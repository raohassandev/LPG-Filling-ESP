...
// ── I2C (new master API, required by esp_io_expander + esp_lcd_touch) ─────────
...
static esp_err_t initExpander() {
    ESP_RETURN_ON_ERROR(
        esp_io_expander_new_i2c_ch422g(s_i2c_bus, kCh422gAddr, &s_expander),
        TAG, "CH422G init");
    ...
    // Reset pulse (LCD + touch resets are active-low)
    esp_io_expander_set_level(s_expander,
        IO_EXPANDER_PIN_NUM_0 | IO_EXPANDER_PIN_NUM_1, 0);
    vTaskDelay(pdMS_TO_TICKS(20));
    // Release reset, turn backlight ON (pin 2)
    esp_io_expander_set_level(s_expander,
