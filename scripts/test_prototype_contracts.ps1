...
Assert-Contains "$root\apps\lpg-expo-app\App.js" 'role.*operator' "Expo app must include User/Operator role."
Assert-Contains "$root\apps\lpg-expo-app\App.js" 'role.*admin' "Expo app must include Admin role."
Assert-Contains "$root\apps\lpg-expo-app\App.js" 'role.*manufacturer' "Expo app must include Manufacturer role."
Assert-Contains "$root\apps\lpg-expo-app\App.js" "tareWeight" "Operator app must include tare weight input."
Assert-Contains "$root\apps\lpg-expo-app\App.js" "Zero Net" "Operator app must include zero-net button."
Assert-Contains "$root\apps\lpg-expo-app\App.js" "netWeightKg" "Operator app must display net weight."
Assert-Contains "$root\apps\lpg-expo-app\App.js" "adminRate" "Admin app must include rate management."
Assert-Contains "$root\apps\lpg-expo-app\App.js" "By Weight" "Operator app must include weight entry mode."
Assert-Contains "$root\apps\lpg-expo-app\App.js" "By Amount" "Operator app must include amount entry mode."
Assert-Contains "$root\apps\lpg-expo-app\App.js" "amount / rateValue" "Amount mode must calculate weight from amount and rate."
Assert-Contains "$firmware\data\index.html" "LPG Controller API" "Firmware web root should be a lightweight API landing page."
